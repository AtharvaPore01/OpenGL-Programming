package com.Atharva_Pore.checkered_board;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private int[] texImage = new int[1];							//in windows and xwindows we write GLuint texImage.
...
*/
	...
	private void oglDisplay()
	{
		...
		//step 1:	allocate buffer directly from native memory.
		ByteBuffer byteBuffer_rectangle_1 = ByteBuffer.allocateDirect(rectangleVertices.length * 4);

		//step 2: 	change the byte order to native byte order.
		byteBuffer_rectangle_1.order(ByteOrder.nativeOrder());

		//step 3:	Create float type buffer and convert our byte type buffer in float
		FloatBuffer positionBuffer_rectangle_1 = byteBuffer_rectangle_1.asFloatBuffer();

		//step 4:	now to the rectangleVertices array in this COOKED Buffer
		positionBuffer_rectangle_1.put(rectangleVertices);

		//step 5:	set the array at the 0th position
		positionBuffer_rectangle_1.position(0);
		GLES32.glBufferData(	GLES32.GL_ARRAY_BUFFER,
								rectangleVertices.length * 4,
								positionBuffer_rectangle_1,
								GLES32.GL_DYNAMIC_DRAW);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
		//draw scene
		...
		//step 1:	allocate buffer directly from native memory.
		ByteBuffer byteBuffer_rectangle_2 = ByteBuffer.allocateDirect(rectangleVertices.length * 4);

		//step 2: 	change the byte order to native byte order.
		byteBuffer_rectangle_2.order(ByteOrder.nativeOrder());

		//step 3:	Create float type buffer and convert our byte type buffer in float
		FloatBuffer positionBuffer_rectangle_2 = byteBuffer_rectangle_2.asFloatBuffer();

		//step 4:	now to the rectangleVertices array in this COOKED Buffer
		positionBuffer_rectangle_2.put(rectangleVertices);

		//step 5:	set the array at the 0th position
		positionBuffer_rectangle_2.position(0);
		GLES32.glBufferData(	GLES32.GL_ARRAY_BUFFER,
								rectangleVertices.length * 4,
								positionBuffer_rectangle_2,
								GLES32.GL_DYNAMIC_DRAW);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
		//draw scene
		GLES32.glDrawArrays(	GLES32.GL_TRIANGLE_FAN,
