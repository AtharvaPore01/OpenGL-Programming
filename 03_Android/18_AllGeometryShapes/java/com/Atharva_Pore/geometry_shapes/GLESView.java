package com.Atharva_Pore.geometry_shapes;
...
import java.lang.Math;										//for mathematics math.h
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private int[] vao_circumscribed_circle = new int[1];
	private int[] vao_circumscribed_circle_triangle = new int[1];
	private int[] vao_circumscribed_rectangle = new int[1];
	private int[] vao_circumscribed_rectangle_circle = new int[1];
	...
	private int[] vbo_circumscribed_circle_position_triangle = new int[1];
	private int[] vbo_circumscribed_circle_position_circle = new int[1];
	private int[] vbo_circumscribed_circle_color = new int[1];
	private int[] vbo_circumscribed_circle_color_triangle = new int[1];
	private int[] vbo_circumscribed_rectangle_position = new int[1];
	private int[] vbo_circumscribed_rectangle_position_circle = new int[1];
	private int[] vbo_circumscribed_rectangle_color = new int[1];
	private int[] vbo_circumscribed_rectangle_color_circle = new int[1];
	...
	private void oglInitialise()
	{
		...
		*/
		...
		final float[] circumscribedCircleColor = new float[]
		{
			1.0f, 1.0f, 0.0f,
			1.0f, 1.0f, 0.0f,
		...
		final float[] rectangleVertice = new float[]
		{
			1.0f, 0.58f, 0.0f,
			-1.0f, 0.58f, 0.0f,
		...
		final float[] circumscribedRectangleColor = new float[]
		{
			1.0f, 1.0f, 0.0f,
			1.0f, 1.0f, 0.0f,
		...
		//triangle
		//create and bind vao
		GLES32.glGenVertexArrays(1, vao_circumscribed_circle_triangle, 0);
		GLES32.glBindVertexArray(vao_circumscribed_circle_triangle[0]);

		//vertices

		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_circle_position_triangle, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_circle_position_triangle[0]);

		//now from here below we are doing 5 steps to convert the triangleVertices array in buffer(compatible to send to glBufferData() function)

		//step 1: allocate the buffer directly from native memory(unmanaged memory)
		ByteBuffer byteBuffer_triangle_vertex = ByteBuffer.allocateDirect(triangleVertices.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 

		//step 2: arrange the byte order of buffer in native byte order
		byteBuffer_triangle_vertex.order(ByteOrder.nativeOrder());

		//step 3: create the float type buffer and convert our byte type buffer in float
		FloatBuffer positionBuffer_triangle = byteBuffer_triangle_vertex.asFloatBuffer();
		...
		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_circle_color_triangle, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_circle_color_triangle[0]);

		//now from here below we are doing 5 steps to convert the triangleVertices array in buffer(compatible to send to glBufferData() function)

		//step 1: allocate the buffer directly from native memory(unmanaged memory)
		ByteBuffer byteBuffer_triangle_color = ByteBuffer.allocateDirect(circumscribedCircleColor.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 
		...
		//step 4: now put your array in this COOKED buffer.
		colorBuffer_triangle.put(circumscribedCircleColor);

		//step 5: set the array at 0th position of the buffer.
		colorBuffer_triangle.position(0);

		GLES32.glBufferData(	GLES32.GL_ARRAY_BUFFER,
								circumscribedCircleColor.length * 4,
		...
		//create and bind vao
		GLES32.glGenVertexArrays(1, vao_circumscribed_circle, 0);
		GLES32.glBindVertexArray(vao_circumscribed_circle[0]);

		//vertices

		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_circle_position_circle, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_circle_position_circle[0]);
		...
		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_circle_color, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_circle_color[0]);

		//now from here below we are doing 5 steps to convert the triangleVertices array in buffer(compatible to send to glBufferData() function)

		//step 1: allocate the buffer directly from native memory(unmanaged memory)
		ByteBuffer byteBuffer_circle_color = ByteBuffer.allocateDirect(circumscribedCircleColor.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 

		//step 2: arrange the byte order of buffer in native byte order
		byteBuffer_circle_color.order(ByteOrder.nativeOrder());

		//step 3: create the float type buffer and convert our byte type buffer in float
		FloatBuffer colorBuffer_circle = byteBuffer_circle_color.asFloatBuffer();

		//step 4: now put your array in this COOKED buffer.
		colorBuffer_circle.put(circumscribedCircleColor);

		//step 5: set the array at 0th position of the buffer.
		colorBuffer_circle.position(0);
		...
		//rectangle
		//create and bind vao
		GLES32.glGenVertexArrays(1, vao_circumscribed_rectangle, 0);
		GLES32.glBindVertexArray(vao_circumscribed_rectangle[0]);

		//vertices

		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_rectangle_position, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_rectangle_position[0]);

		//now from here below we are doing 5 steps to convert the triangleVertices array in buffer(compatible to send to glBufferData() function)

		//step 1: allocate the buffer directly from native memory(unmanaged memory)
		ByteBuffer byteBuffer_rectangle_vertex = ByteBuffer.allocateDirect(rectangleVertice.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 

		//step 2: arrange the byte order of buffer in native byte order
		byteBuffer_rectangle_vertex.order(ByteOrder.nativeOrder());

		//step 3: create the float type buffer and convert our byte type buffer in float
		FloatBuffer positionBuffer_rectangle = byteBuffer_rectangle_vertex.asFloatBuffer();

		//step 4: now put your array in this COOKED buffer.
		positionBuffer_rectangle.put(rectangleVertice);

		//step 5: set the array at 0th position of the buffer.
		positionBuffer_rectangle.position(0);

		GLES32.glBufferData(	GLES32.GL_ARRAY_BUFFER,
								rectangleVertice.length * 4,
		...
		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_rectangle_color, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_rectangle_color[0]);

		//now from here below we are doing 5 steps to convert the triangleVertices array in buffer(compatible to send to glBufferData() function)

		//step 1: allocate the buffer directly from native memory(unmanaged memory)
		ByteBuffer byteBuffer_rectangle_color = ByteBuffer.allocateDirect(circumscribedRectangleColor.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 
		...
		//step 4: now put your array in this COOKED buffer.
		colorBuffer_rectangle.put(circumscribedRectangleColor);

		//step 5: set the array at 0th position of the buffer.
		colorBuffer_rectangle.position(0);

		GLES32.glBufferData(	GLES32.GL_ARRAY_BUFFER,
								circumscribedRectangleColor.length * 4,
		...
		//circle
		//create and bind vao
		GLES32.glGenVertexArrays(1, vao_circumscribed_rectangle_circle, 0);
		GLES32.glBindVertexArray(vao_circumscribed_rectangle_circle[0]);

		//vertices

		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_rectangle_position_circle, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_rectangle_position_circle[0]);
		...
		//create and bind vbo
		GLES32.glGenBuffers(1, vbo_circumscribed_rectangle_color_circle, 0);
		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_circumscribed_rectangle_color_circle[0]);

		//now from here below we are doing 5 steps to convert the triangleVertices array in buffer(compatible to send to glBufferData() function)

		//step 1: allocate the buffer directly from native memory(unmanaged memory)
		ByteBuffer byteBuffer_color_circle = ByteBuffer.allocateDirect(circumscribedRectangleColor.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 

		//step 2: arrange the byte order of buffer in native byte order
		byteBuffer_color_circle.order(ByteOrder.nativeOrder());

		//step 3: create the float type buffer and convert our byte type buffer in float
		FloatBuffer colorBuffer_Circle = byteBuffer_color_circle.asFloatBuffer();

		//step 4: now put your array in this COOKED buffer.
		colorBuffer_Circle.put(circumscribedRectangleColor);
	...
	private void oglGenCircleInsideTriangle()
	{
		//OGLCircleInsideTriangle Variables
		double radius, a, b, c;
		double Perimeter;
		double Area_Of_Triangle, x_center, y_center;
		float[] circleVertices = new float[3];

		//code
		//Distance Between Vertices Of The Triangle
		a = Math.sqrt((Math.pow((-1.0 - 0.0), 2) + Math.pow((-0.6 - 1.2), 2)));
		b = Math.sqrt((Math.pow((1.0 - (-1.0)), 2) + Math.pow((-0.6 - (-0.6)), 2)));
		c = Math.sqrt((Math.pow((0.0 - 1.0), 2) + Math.pow((1.2 - (-0.6)), 2)));
		
		//Semi Perimeter
		Perimeter = (a + b + c) / 2;

		//Area Of Trianle Using Heron's Formula
		Area_Of_Triangle = Math.sqrt(Perimeter * (Perimeter - a) * (Perimeter - b) * (Perimeter - c));

		//Radius Of Circle
		radius = Area_Of_Triangle / Perimeter;
		
		//Center Of The Circle
		x_center = ((a * 1.0) + (b * (0.0)) + (c * (-1.0))) / (a + b + c);
		y_center = ((a * (-0.6)) + (b * (1.2)) + (c * (-0.6))) / (a + b + c);
		...
		for (float angle = 0.0f; angle < (2.0f * Math.PI); angle = angle + 0.01f)
		{
			...
			//step 1: allocate the buffer directly from native memory(unmanaged memory)
			ByteBuffer byteBuffer_circle_position = ByteBuffer.allocateDirect(circleVertices.length * 4);	//java doesn't have sizeof() so, size of an array = array length * size of type 

			//step 2: arrange the byte order of buffer in native byte order
			byteBuffer_circle_position.order(ByteOrder.nativeOrder());

			//step 3: create the float type buffer and convert our byte type buffer in float
			FloatBuffer positionBuffer_circle = byteBuffer_circle_position.asFloatBuffer();
			...
			GLES32.glBufferData(	GLES32.GL_ARRAY_BUFFER,
									circleVertices.length * 4,
									positionBuffer_circle,
									GLES32.GL_DYNAMIC_DRAW);

			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);

			//draw scene
			//GLES32.glPointSize(1.5f);
	...
	private void oglGenCircumscribedRectangle()
	{
		//variable declaration
		double Number_Of_Sides = 4.0;
		double radius;
		double Top, Bottom, Left, Right, Area_of_Rectangle;
		float[] circleVertices = new float[3];

		//code
		
		//Rectangle's Sides
		Top = Math.sqrt((Math.pow((-1.0 - 1.0), 2) + Math.pow((0.58 - 0.58), 2)));
		Bottom = Math.sqrt((Math.pow((1.0 - (-1.0)), 2) + Math.pow((-0.6 - (-0.6)), 2)));
		Left = Math.sqrt((Math.pow((1.0 - 1.0), 2) + Math.pow((-0.58 - 0.6), 2)));
		Right = Math.sqrt((Math.pow((-1.0 - (-1.0)), 2) + Math.pow((-0.6 - 0.58), 2)));

		//Area Of Rectangle
		Area_of_Rectangle = (Top + Bottom + Left + Right) / 2;

		//Radius
		radius = (Math.sqrt((Math.pow(Bottom, 2)) + (Math.pow(Right, 2))) / 2);
		...
		for (float angle = 0.0f; angle < (2.0f * Math.PI); angle = angle + 0.01f)
		{
			circleVertices[0] = (float)((Math.cos(angle) * radius));
			circleVertices[1] = (float)((Math.sin(angle) * radius));
			circleVertices[2] = 0.0f;
	...
	private void oglUninitialise()
	{
		...
		if (vbo_circumscribed_circle_color_triangle[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_circle_color_triangle, 0);
			vbo_circumscribed_circle_color_triangle[0] = 0;
		}
		if (vbo_circumscribed_circle_color[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_circle_color, 0);
			vbo_circumscribed_circle_color[0] = 0;
		}
		if (vbo_circumscribed_circle_position_triangle[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_circle_position_triangle, 0);
			vbo_circumscribed_circle_position_triangle[0] = 0;
		}
		if (vbo_circumscribed_circle_position_circle[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_circle_position_circle, 0);
			vbo_circumscribed_circle_position_circle[0] = 0;
		}
		if (vbo_circumscribed_rectangle_color[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_rectangle_color, 0);
			vbo_circumscribed_rectangle_color[0] = 0;
		}
		if (vbo_circumscribed_rectangle_color_circle[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_rectangle_color_circle, 0);
			vbo_circumscribed_rectangle_color_circle[0] = 0;
		}
		if (vbo_circumscribed_rectangle_position[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_rectangle_position, 0);
			vbo_circumscribed_rectangle_position[0] = 0;
		}
		if (vbo_circumscribed_rectangle_position_circle[0] != 0)
		{
			GLES32.glDeleteBuffers(1, vbo_circumscribed_rectangle_position_circle, 0);
			vbo_circumscribed_rectangle_position_circle[0] = 0;
		}
		...
		if (vao_circumscribed_circle[0] != 0)
		{
			GLES32.glDeleteVertexArrays(1, vao_circumscribed_circle, 0);
			vao_circumscribed_circle[0] = 0;
		}
		if (vao_circumscribed_circle_triangle[0] != 0)
		{
			GLES32.glDeleteVertexArrays(1, vao_circumscribed_circle_triangle, 0);
			vao_circumscribed_circle_triangle[0] = 0;
		}
		if (vao_circumscribed_rectangle[0] != 0)
		{
			GLES32.glDeleteVertexArrays(1, vao_circumscribed_rectangle, 0);
			vao_circumscribed_rectangle[0] = 0;
		}
		if (vao_circumscribed_rectangle_circle[0] != 0)
		{
			GLES32.glDeleteVertexArrays(1, vao_circumscribed_rectangle_circle, 0);
			vao_circumscribed_rectangle_circle[0] = 0;
		}
