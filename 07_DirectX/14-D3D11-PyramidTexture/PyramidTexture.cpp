...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName_ap,
		TEXT("Direct3D11 : Textured Pyramid Animating"),
...
HRESULT d3dInitialise(void)
{
	...
	hr_ap = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Pyramid_Texture);
	if (FAILED(hr_ap))
	{
		fopen_s(&gpFile_ap, gszLogFileName_ap, "a+");
	...
	{
		fopen_s(&gpFile_ap, gszLogFileName_ap, "a+");
		fprintf_s(gpFile_ap, "d3dResize() is Failed.\n");
		fclose(gpFile_ap);
		return(hr_ap);
	}
	else
