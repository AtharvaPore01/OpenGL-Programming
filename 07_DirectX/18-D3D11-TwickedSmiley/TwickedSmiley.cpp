...
ID3D11SamplerState *gpID3D11SamplerState_Rectangle_Texture = NULL;
	...
	//twicked smiley change
D3D11_MAPPED_SUBRESOURCE mappedSubresourceRectTexture;	//twicked smiley change
...
*/
	...
	hr_ap = gpID3D11Device->CreateBuffer(&bufferDescRectPosition, NULL,
	...
	hr_ap = gpID3D11Device->CreateBuffer(&bufferDescRectTexture, NULL,
	...
	//create texture
	hr_ap = LoadD3DTexture(L"Smiley.bmp", &gpID3D11ShaderResourceView_Rectangle_Texture);
	if (FAILED(hr_ap))
	{
		fopen_s(&gpFile_ap, gszLogFileName_ap, "a+");
	...
	hr_ap = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Rectangle_Texture);
	if (FAILED(hr_ap))
	{
		fopen_s(&gpFile_ap, gszLogFileName_ap, "a+");
...
void d3dDisplay(void)
{
	...
	gpID3D11DeviceContext->PSSetShaderResources(	0,
													1,
													&gpID3D11ShaderResourceView_Rectangle_Texture);
	gpID3D11DeviceContext->PSSetSamplers(	0,
											1,
											&gpID3D11SamplerState_Rectangle_Texture);
