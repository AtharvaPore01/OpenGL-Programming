...
BOOL LoadTexture(GLuint *Texture, TCHAR imageResourceID[])
{
	...
	//code
	//Taking Handle Of A Given BITMAP ID
	...
	if(hBitmap)
	{
		...
		//Getting The Data Of The Bitmap Using SDK Function GetObject
		GetObject(hBitmap, sizeof(BITMAP), &bmp);

		glPixelStorei(GL_UNPACK_ALIGNMENT, 4);

		//Opengl Common Construct
		//1.	Generate Something On The Device(Allocate The Memory) And Get Unique ID Of That Memory

		glGenTextures(1, Texture);

		//2.	Bind That Obtained Block To The Desired DataStructure For Which You Are Using That Memory Block

		glBindTexture(GL_TEXTURE_2D, *Texture);

		//3.	Set The Parameters / State Of That DS
