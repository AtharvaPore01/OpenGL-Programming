...
GLfloat Reactangle_Sides[] = {1.0f, 0.9f, 0.8f, 0.7f, 0.6f, 0.5f, 0.4f, 0.3f};
...
void OGLTriangle(void)
{
	...
	for(i = 0, j = 0; i < sizeof(Reactangle_Sides)/sizeof(Reactangle_Sides[0]), j < 27; i++, j = j + 3)
	{
		glColor3f(colors[j], colors[j + 1], colors[j + 2]);
		//Vertex 1
		glVertex2f(0.0f, Reactangle_Sides[i]);
		glVertex2f(-Reactangle_Sides[i], -Reactangle_Sides[i]);
		//Vertex 2
		glVertex2f(-Reactangle_Sides[i], -Reactangle_Sides[i]);
		glVertex2f(Reactangle_Sides[i], -Reactangle_Sides[i]);
		//Vertex 3
