...
bool oglLoadTexture(GLuint *texture, const char *path)
{
	...
	//code
	pImageData = SOIL_load_image(	path,
									&width,
									&height,
									0,
									SOIL_LOAD_RGB);
	...
	if(pImageData == NULL)
	{
		printf("ERROR::oglLoadTexture() : pImageData is NULL.\n");
		bResult = false;
		return(bResult);
	}
	else
	{
		printf("SUCCESS::oglLoadTexture() : pImageData is not NULL.\n");
		bResult = true;
	}
