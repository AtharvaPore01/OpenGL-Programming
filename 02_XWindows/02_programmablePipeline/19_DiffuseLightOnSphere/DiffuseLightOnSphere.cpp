...
GLuint LdUniform;
GLuint KdUniform;
GLuint LKeyIsPressed_Uniform;
GLuint lightPosition_Uniform;

//flag for light toggling
bool bLight = false;
...
void oglInitialise(void)
{
	...
	const GLchar *vertexShaderSourceCode = 
		...
		"{" \
			...
			"{" \
				...
				"diffuse_light = u_Ld * u_Kd * max(dot(light_direction, transformed_normal), 0.0);" \
			"}" \
	...
	*/
	...
	projectionUniform 		= glGetUniformLocation(	gShaderProgramObject, 
													"u_projection_matrix");
	LKeyIsPressed_Uniform 	= glGetUniformLocation(	gShaderProgramObject,
													"u_LKeyPressed");
...
{	
	...
	if(bLight)
	{
		//notify shader that "L" key is pressed.
		glUniform1i(LKeyIsPressed_Uniform, 1);
		//send light intensity
		glUniform3f(LdUniform, 0.5f, 0.2f, 0.7f);
		//send coeff. of material's diffuse reflectivity
		glUniform3f(KdUniform, 1.0f, 1.0f, 1.0f);
		//send light position
		glUniform4f(lightPosition_Uniform, 100.0f, 100.0f, 100.0f, 1.0f);
	}
	else
	{
		//notify shader that "L" key isn't pressed.
		glUniform1i(LKeyIsPressed_Uniform, 0);
	}
