...
void oglCreateWindow(void)
{
	...
	XStoreName(gpDisplay, gWindow, "Concentric Circle");
...
void concentric_circle(void)
{
	//variable declaration
	int 	i, j;
	GLfloat	radius[] 	= 	{1.0f, 0.9f, 0.8f, 0.7f, 0.6f, 0.5f, 0.4f, 0.3f};
	...
	{
		for(i = 0, j = 0; i < sizeof(radius)/sizeof(radius[0]), j < 27; i++, j = j + 3)
		{
			glColor3f(colors[j], colors[j + 1], colors[j + 2]);
			glVertex2f(cosf(angle) * radius[i], sinf(angle) * radius[i]);
		}
